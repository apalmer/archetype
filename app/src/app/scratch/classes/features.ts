...
rage.on=function(player){
    ...
    return {status:'rage',state:'off'}}
    else{
    let rages=player.resources.find(b=>b.name==='Rage')
    if(rages.min===0){alert('the rage no longer comes')}
    else{
    rages.min-=1
...
ki.start=function(player){
  if (player.resources.find(b=>b.name='Ki')){
    player.resources.find(b=>b.name='Ki').max=player.TotalLevel
  }
  else{

  player.resources.push({name:'Ki',min:player.TotalLevel,max:player.TotalLevel})}
